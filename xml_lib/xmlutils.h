...
inline void die(const std::string& msg, const std::string& token1 = "", const std::string& token2 = "",
...
inline std::string ToString(double d, int precision)
{
    char buf[1024];
...
inline std::vector<std::string>& Split(const std::string& input, std::vector<std::string>& output,
...
inline std::string UnescapeCharacters(const std::string& str)
{
...
inline std::vector<std::string>& Unquote(std::vector<std::string>& strs)
{
...
inline char* TrimTrailingWhitespace(char* str)
{
    char* start = str;
...
inline std::string& AppendSlash(std::string& path, bool local = false)
{
...
inline std::string& AppendPath(
    std::string& path, const std::string& subpath, bool appendSlash = true, bool local = false)
{
    path += subpath;
    if (appendSlash) {
...
inline bool ControlCIssued()
{
    return ControlCHandler::ControlCIssued();
}

inline bool ParseBoolean(const char* s, bool* exactMatch = nullptr)
{
    assert(s);
...
inline __int64_t ParseInteger(const char* s, bool* exactMatch = nullptr)
{
    assert(s);
    char* e;
...
inline double ParseReal(const char* s, bool* exactMatch = 0)
{
    assert(s);
    char* e;
    double d = strtod(s, &e);
